...
FBTextureManager::FBTextureManager() : clearTextureExtension(true) {
}

FBTextureManager::~FBTextureManager() {
	textures.clear();
}

void FBTextureManager::add(const string& texture) {
	GLuint tmpId;
	glGenTextures(1, &tmpId);
	textures[texture] = tmpId;
}

void FBTextureManager::createTexture(const string& texture, const string filePath,unsigned w, unsigned h, GLuint filter, GLuint type, GLuint type_2, bool depth = false)
{
//	SDL_Surface *surface;
	add(texture);
...
void FBTextureManager::createRGBA16F3DTexture(const string& texture, glm::vec3 dim, GLuint filter, GLuint wrap) {
	GLuint tex;
	add(texture);
...
void FBTextureManager::clear3Dtexture(GLuint texture) {
...
void FBTextureManager::createRGBA3DTexture(const string& texture, glm::vec3 dim, GLuint filter, GLuint wrap) {
	GLuint tex;
	add(texture);
	glBindTexture(GL_TEXTURE_3D, textures[texture]);
	glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_MAG_FILTER, filter);
...
GLuint FBTextureManager::operator[] (const string& texture) {
